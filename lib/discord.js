 ...
 */
export async function uploadChunk(buffer, filename, chunkIndex, totalChunks, fileId, mimeType) {
  const channelId = process.env.DISCORD_CHANNEL_ID;
  const chunkName = totalChunks > 1
    ? `${filename}.part${String(chunkIndex).padStart(3, '0')}`
    : filename;

  // Embed metadata in message content for later retrieval
  const metadata = JSON.stringify({
    _cv: 1, // CloudVault version marker
    fileId,
    fileName: filename,
 ...
 */
export async function storeMetadata(fileId, fileName, mimeType, size, storageType, chunkInfo) {
  const channelId = process.env.DISCORD_CHANNEL_ID;
  const metadata = JSON.stringify({
    _cv: 1,
    _meta: true,
  ...
  while (fetched < limit) {
    ...
    for (const msg of messages) {
      try {
        ...
        if (meta._meta) {
          // Metadata-only record (Telegram/bot file)
          if (!files.has(meta.fileId)) {
            files.set(meta.fileId, {
              ...
              _discordMetaMsgId: msg.id, // Track the metadata message for deletion
            });
          }
        } else {
          // Discord file chunk (has attachment)
          if (!files.has(meta.fileId)) {
            files.set(meta.fileId, {
              id: meta.fileId,
              name: meta.fileName,
