...
/**
 * Attach a thumbnail reference to a cached file
 * @param {string} fileId
 * @param {{messageId: string, channelId: string}} thumbnail
 */
export function setThumbnail(fileId, thumbnail) {
...
export async function deleteFileFromStorage(file) {
  ...
  // Delete thumbnail chunk if present
  if (file.thumbnail) {
    try { await discord.deleteMessage(file.thumbnail.messageId, file.thumbnail.channelId); } catch { }
  }
}
